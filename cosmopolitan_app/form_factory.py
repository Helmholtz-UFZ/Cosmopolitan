...
class FormFactory:
    ...
    def set_model(self, form_data: dict) -> None:
        ...
        try:
            ...
            # Always set pymodel to reflect the current form state, even when
            # invalid. model_construct skips all validators, preventing cascade
            # failures from model validators with multi-field loc_tuples where
            # resetting only locs[0] to default doesn't fix the constraint.
            self.pymodel = ModelWebsite.model_construct(**model_dict)
            raise
